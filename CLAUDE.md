...
```bash
docker compose up -d postgres postgres-vn postgres-mx      # 리전별 PostgreSQL (esgo/ 상위)
...
npm run db:pull:vn       # 베트남 리전 DB → Prisma 스키마 동기화
npm run db:pull:mx       # 멕시코 리전 DB → Prisma 스키마 동기화
...
src/actions/              Server Actions (도메인별 폴더, 함수당 1파일)
  ...
  content/                콘텐츠 (문서, ESG, 보고서, 샘플 PDF 관리)
...
src/lib/                  싱글턴 인프라 (auth, prisma, regions, env, logger, action, format, constants, utils, s3)
prisma/kr/                한국 리전 Prisma 스키마 + config (DATABASE_URL_KR)
prisma/vn/                베트남 리전 Prisma 스키마 + config (DATABASE_URL_VN)
prisma/mx/                멕시코 리전 Prisma 스키마 + config (DATABASE_URL_MX)
```
...
4. **멀티 리전 Prisma**: `getPrismaClient(regionId)` — ClientMap 패턴으로 KR/VN/MX 리전별 캐시된 PrismaClient
...
**Admin은 자체 DB가 없다. 리전별 Platform의 PostgreSQL을 공유한다.**

```
KR: ../esg-platform  →  db push  →  Admin: npm run db:pull:kr
VN: ../esgo-vietnam   →  db push  →  Admin: npm run db:pull:vn
MX: ../esgo-mexico    →  db push  →  Admin: npm run db:pull:mx
전체: npm run db:pull:all → 변경된 prisma/{kr,vn,mx}/schema.prisma 반드시 커밋
```
...
프로덕션: https://admin.esgohq.com. `ghcr.io/team-dogfoot/esgo-admin`. GitHub Actions → lint/tsc → Docker → GHCR → K8s 매니페스트 갱신 → ArgoCD. CI/CD에서 DB 접근 없음 — 커밋된 `prisma/{kr,vn,mx}/schema.prisma`로 `npm run db:generate:all`만 실행.
