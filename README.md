...
- **콘텐츠 관리**: 문서 목록, ESG 데이터(카테고리 통계, 항목 랭킹, 워크스페이스 진행도), 보고서 목록, 샘플 PDF 관리(리전별 S3 메타 조회, 원클릭 재생성)
...
│   │       └── content/        #     콘텐츠 대시보드 + documents, esg-data, reports, sample-pdf
│   ├── (auth)/login/           # 로그인 페이지
...
│   ├── content/                # document-table, esg-overview-table, report-table, esg-category-stats, esg-item-rankings, sample-pdf-manager
...
│   ├── s3.ts                   # AWS S3 클라이언트, getSamplePdfMeta() (HeadObject)
...
| `INTERNAL_API_KEY` | 내부 API 인증 키 (샘플 PDF 재생성) | `change-me-in-production` |
