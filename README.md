...
- **워크스페이스 관리**: 목록 조회, 상세 정보(7탭), 크레딧 수동 조정(지급/차감/일괄), 구독 플랜 변경
- **사용자 관리**: 전체 사용자 목록, 상세(소속 워크스페이스, 활동 상태)
- **빌링 관리**: 구독 현황, 결제 내역, 크레딧 원장, 크레딧 소비 추이 차트
- **AI 서비스 모니터링**: 파이프라인 사용 현황, 에러 목록, 워크스페이스/기능별 크레딧 분석, 소비 추이
...
│   │   ├── batch-adjust-credits.ts  # 일괄 크레딧 조정
...
│   ├── prisma.ts               # 리전별 PrismaClient 팩토리 + PlanCode/CreditType/PaymentStatus/SubscriptionStatus re-export
...
| `CreditLedger` | 크레딧 거래 이력 (INITIAL/MONTHLY/PURCHASE/CONSUME/REFUND/ADMIN) |
...
- **Prisma enum re-export**: `PlanCode`, `CreditType`, `PaymentStatus`, `SubscriptionStatus`를 `src/lib/prisma.ts`에서 re-export. Zod `nativeEnum`으로 런타임 검증
